            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-zinc-200 dark:border-white/10 bg-zinc-50 dark:bg-white/5">
                        <th class="text-left px-4 py-3 font-medium text-zinc-500 dark:text-zinc-400 min-w-[200px]">
                        ...
                        <th class="text-left px-4 py-3 font-medium text-zinc-500 dark:text-zinc-400 w-44">Categoría</th>
                ...
                <tbody>
                    ...
                    <tr class="border-b border-zinc-200 dark:border-white/10 hover:bg-zinc-50 dark:hover:bg-white/5 transition-colors group">
                        <td class="px-4 py-3 font-medium text-zinc-800 dark:text-white">{{ $sub->nombre }}</td>

                        <td class="px-4 py-3">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-zinc-100 text-zinc-700 dark:bg-white/10 dark:text-zinc-300">
                                {{ $sub->categoria->nombre ?? '—' }}
                            </span>
                        </td>

                        <td class="px-4 py-3">
                            <span class="text-zinc-500 dark:text-zinc-400 truncate block max-w-md">{{ $sub->descripcion ?? '—' }}</span>
                        </td>
                        ...
                        <td class="px-4 py-3 text-center">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $sub->activo ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' : 'bg-zinc-100 text-zinc-800 dark:bg-zinc-900/30 dark:text-zinc-400' }}">
                                <span class="w-1.5 h-1.5 rounded-full mr-1.5 {{ $sub->activo ? 'bg-green-500' : 'bg-zinc-400' }}"></span>
                    ...
                    <tr>
                        <td colspan="6" class="px-4 py-12 text-center text-zinc-400">
                            <div class="flex flex-col items-center gap-3">
                                ...
                                <p>No se encontraron subcategorías con los filtros aplicados.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
